...
public class TestinUIAutomator
{
	...
	private void startServer()
	{
		...
		while(!isBindSuccess)
		{
			...
			{
				UnityEngine.Debug.LogError ("failed to bind socket on port " + port + " error is " + e.ToString ());
				port++;
				if (port >= TESTIN_UIAUTOMATOR_PORT + TESTIN_UIAUTOMATOR_PORT_RANGE)
				{
			        ...
			        {
			           UnityEngine.Debug.LogError ("Thread sleep error : " + e1.ToString ());
			        }
				}
			}
		}
	...
	private void AcceptCallback(IAsyncResult ar)
	{
		...
		// save the client request to the byte array
		...
		//the split between the header and the body
